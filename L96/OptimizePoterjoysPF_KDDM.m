function [x,yAll,MSE,traceP,z] = OptimizePoterjoysPF_KDDM(n,T,var_y,skip,Gap,NeAll,LocRadAll,aAll,Save,EXIT)
dt = 0.05;
t = 0:dt:T;
Steps = length(t);
%% ------------------------------------------
...
for zz=1:length(NeAll)
    ...
    for xx=1:length(LocRadAll)
        ...
        for tt = 1:length(aAll)
            a = aAll(tt);
            fprintf('Poterjoys PF, KDDM, Ne = %g, loc-rad = %g,  a = %g\n',Ne,locRad,a)
           [x,traceP] = PoterLocPF(a*Ne,locRad,Ne,xpf,z,var_y,H,F,Gap,Steps,dt,1);
           MSE = mean((x(:,Gap+1:Gap:end) - yAll(:,Gap+1:Gap:end)).^2);
           traceP = traceP'/n;
            if Save == 1
                Filename = strcat('PoterjoysPF_KDDM_Gap_',num2str(Gap),'_Ne_',num2str(Ne), ...
