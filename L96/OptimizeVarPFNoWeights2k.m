function [xAll,yAll,MSE,traceP,z,RSave] = OptimizeVarPFNoWeights2k(n,T,var_y,skip,Gap,NeAll,ClocRadAll,inflAll,Save,EXIT)
...
for zz=1:length(NeAll)
    ...
    for pp = 1:length(ClocRadAll)
        ...
        for tt = 1:length(inflAll)
            ...
            [xAll,traceP,~]=myVarPFNoWeights2k(infl,Cloc,Ne,muo,Lb,z,R,H,F,Gap,Steps,dt);
            MSE = mean( (xAll(:,Gap+1:Gap:end)-yAll(:,Gap+1:Gap:end)).^2 );
            if Save == 1
                Filename = strcat('varPFNoWeights2k_Gap_',num2str(Gap),'_Ne_',num2str(Ne), ...
