...
function [xmpf,xa,eflag] = pf_update(xpf,Nx,Ne,H,Y,roi,alpha,oborder,var_y,kddm_flag)
...
for i = oborder % Observation loop
  ...
  for n = 1:Ne
    ...
    % Calculate w
    d1 = (Y(i)-Hi*xpf{n}')/sqrt(2*var_y);   % Normalized innovation
    wn = exp( - d1*d1 )./sqrt(2*pi);        % p(y|x_n)
    w(n) = ( wn - 1 )*alpha + 1;

    % Calculate wo
    d2 = (Y(i)-Hi*xpfo{n}')/sqrt(2*var_y);   % Normalized innovation
  ...
  var_a = 0;
  for n = 1:Ne
    var_a = var_a + wo(n,:).*(xpfo{n}-xmpf).*(xpfo{n}-xmpf)*Ne/(Ne-1);
  end
